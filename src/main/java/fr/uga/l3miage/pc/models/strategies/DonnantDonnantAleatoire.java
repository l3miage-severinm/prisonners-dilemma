...
public class DonnantDonnantAleatoire implements SimpleStrategy {
    ...
    public boolean doStrategy(Tour[] historique, int indexTourEnCours, int idJoueur) throws JoueurAPasJoueException {
        return (random.nextInt(100) < 20) ? random.nextBoolean() : (idJoueur == 1) ? historique[indexTourEnCours - 1].getJoueur2Coopere() : historique[indexTourEnCours].getJoueur1Coopere();
    }
}
